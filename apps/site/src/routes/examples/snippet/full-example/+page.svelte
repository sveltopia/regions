<script lang="ts">
  ...
  // Cast snippets to Snippet<[]> which is accepted by RegionContent
  useLayoutRegions({
    productHeader: productHeaderSnippet as Snippet<[]>,
    settingsHeader: settingsHeaderSnippet as Snippet<[]>,
    statsHeader: statsHeaderSnippet as Snippet<[]>
  });
  ...
  <RegionIndicator label="Stats Header Region (Snippet with Reactive State)" color="blue">
    <div class="space-y-4">
      <div>
        ...
        <p class="text-sm text-muted-foreground">Real-time data with full reactivity</p>
      </div>
  ...
  <RegionIndicator label="Product Header Region (Snippet)" color="orange">
    <div class="flex items-center justify-between">
      <h2 class="mt-0 mb-0 text-3xl font-bold">
        Svelte is <span class="font-bold text-indigo-600 dark:text-indigo-400"
          >{adjectives[currentIndex]}</span
        >
      </h2>
      <button
        onclick={nextAdjective}
        class="cursor-pointer rounded bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600"
      >
        Next
      </button>
    </div>
...
{#snippet settingsHeaderSnippet()}
  <RegionIndicator label="Settings Header Region (Snippet Wrapping Component)" color="green">
    <SettingsPanel
      {theme}
      {notificationsEnabled}
...
<div class="not-prose mb-8 rounded-lg border bg-indigo-500/5 p-6">
  ...
  <p class="mt-4 text-sm text-muted-foreground">
    ...
    consumer that accepts Snippets and an example layout file with snippet definitions and full state
    access.
  </p>
</div>

<p>
  Snippets are the most powerful and flexible strategy for regions. They give you <strong
    >full access to page state, props, and context</strong
  > while remaining type-safe and reactive.
</p>
...
<ul>
  <li>
    <strong>1. User Interaction</strong> - Snippets can handle events and mutate page state
    directly, making highly interactive UIs trivial.
    <em>(See Product Header - click "Next" to cycle adjectives)</em>
  </li>
  <li>
    <strong>2. Component Wrapping</strong> - Snippets can wrap entire components, giving them access
    to page state and handlers without prop drilling through your layout.
    <em>(See Settings Header - full component with page state access)</em>
  </li>
  <li>
    <strong>3. Reactive State Access</strong> - Snippets have direct access to all page state,
    props, and context. When state changes, snippets automatically re-render.
...
<p>
  Snippets solve a specific problem: <strong
    >"I need the layout to control positioning and chrome, while each page controls what content
    goes there"</strong
  > - whether that's simple varying markup or deeply interactive UI with full page state access.
</p>

<div class="not-prose mb-6 space-y-3">
  <h4>Example Use Cases:</h4>
  <div class="rounded-lg border bg-card p-4">
    <h4 class="mb-2 text-sm font-semibold">Page Actions Bar</h4>
    <ul class="space-y-1 text-sm text-muted-foreground">
      <li>
        • <strong>Layout:</strong> "Page actions always appear top-right, styled consistently"
      </li>
      <li>
        • <strong>Page snippet:</strong> "<em>This</em> page needs
        <strong>Save + Delete</strong>, connected to my form state. Another page needs
        <strong>Publish + Archive</strong>."
      </li>
      <li>
        • <strong>Alternative:</strong> Every page duplicates positioning/styling OR prop drilling hell
      </li>
    </ul>
  </div>

  <div class="rounded-lg border bg-card p-4">
    <h4 class="mb-2 text-sm font-semibold">Contextual Sidebar</h4>
    <ul class="space-y-1 text-sm text-muted-foreground">
      <li>
        • <strong>Layout:</strong> "Sidebar appears on right, collapsible, consistent behavior"
      </li>
      <li>
        • <strong>Page snippet:</strong> "<em>This</em> page's sidebar needs
        <strong>filters</strong> that update my data table. Another page's sidebar needs
        <strong>navigation links</strong>."
      </li>
      <li>
        • <strong>Why snippet?</strong> Filter state lives on page, but sidebar position is layout concern
      </li>
    </ul>
  </div>

  <div class="rounded-lg border bg-card p-4">
    <h4 class="mb-2 text-sm font-semibold">Dynamic Breadcrumbs</h4>
    <ul class="space-y-1 text-sm text-muted-foreground">
      <li>
        • <strong>Layout:</strong> "Breadcrumbs always at top, styled, positioned"
      </li>
      <li>
        • <strong>Page snippet:</strong> "My breadcrumbs are interactive (click to filter), need page
        state"
      </li>
    </ul>
  </div>
...
<KeyConcept>
  <strong>The pattern:</strong> Layout controls the "chrome" (position, container, transitions), snippet
  provides the "meat" (interactive content with page state access). This separation of concerns keeps
  your layouts DRY while giving pages maximum flexibility.
</KeyConcept>

<h3>Why TypeScript Only for Snippets?</h3>
<p>
  Snippets are typed at compile-time through their signature. Runtime validation with Valibot/Zod
  would be redundant since the snippet signature already enforces types.
</p>
...
<div class="space-y-8">
  <div class="space-y-6">
    <div>
      ...
      <KeyConcept>
        <strong>Notice the 3 patterns:</strong> Snippets can handle user interaction (example 1), wrap
        full components (example 2), and access reactive state (example 3). All have access to page state
        via closure.
      </KeyConcept>
    </div>
    <CodeViewer
  ...
  <div class="space-y-6">
    <p class="text-sm font-medium">
      <strong>Step 2:</strong> Consume snippets in your layout &mdash; the layout controls positioning
      and styling, while the snippets provide interactive content with full page state access.
    </p>
    <CodeViewer
