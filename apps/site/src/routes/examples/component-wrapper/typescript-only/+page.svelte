...
<div class="not-prose mb-8 rounded-lg border bg-indigo-500/5 p-6">
  ...
  <p class="mt-4 text-sm text-muted-foreground">
    ...
    <strong>TypeScript only</strong> validator, then define your fields (username, bio).
  </p>
</div>
...
<ul>
  ...
  <li>
    <strong>TypeScript only</strong> - Compile-time type checking with no runtime validation overhead
    (0kb validator bundle)
  </li>
  <li>
...
<div class="space-y-8">
  <div class="space-y-6">
    <p class="text-sm font-medium">1. Define your type with a TypeScript interface:</p>
    <CodeViewer
  ...
  <div class="space-y-6">
    <div>
      ...
      <KeyConcept>
        ...
        <code class="not-prose">useLayoutRegions()</code>, which sets the region data for
        consumption elsewhere in your layout (no runtime validation).
      </KeyConcept>
    </div>
    <CodeViewer
  ...
  <div class="space-y-6">
    <p class="text-sm font-medium">
      ...
      handles calling <code class="not-prose">useLayoutRegions()</code>, which makes the data
