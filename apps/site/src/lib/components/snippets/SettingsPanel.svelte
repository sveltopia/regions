<script lang="ts">
  ...
  let { theme, onThemeChange, notificationsEnabled, onNotificationToggle } =
    $props<{
...
<div
  class="space-y-4 rounded-lg border-2 p-4 transition-all duration-300 {themeClasses()}">
  <div>
    <h3 class="text-lg font-semibold mb-3 {labelClasses()}">User Settings</h3>
    <p class="text-sm {labelClasses()}">
      This panel demonstrates <strong class="{labelClasses()}">real interactivity</strong>: the theme
      selector changes this section's appearance in real-time, and the
      notification toggle controls whether theme change toasts appear.
    </p>
  </div>
  ...
  <div class="space-y-3">
    ...
    <div
      class="flex items-center justify-between rounded-lg border p-3 transition-colors {cardClasses()}">
      <div>
        <div class="text-sm font-medium {labelClasses()}">
          Toast Notifications
        </div>
