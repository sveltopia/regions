<script lang="ts">
  ...
  const questions = [
    {
      id: 1,
      question: 'Do you need to pass content from child pages to parent layouts?',
    ...
    },
    {
      ...
      description: 'Or would you prefer CLI scaffolding and automated type-safe patterns?',
      choices: [
        {
          value: 'yes',
          label: 'Prefer manual',
    ...
    },
    {
      ...
      description: 'Custom HTML, reactive state, dynamic content, page-specific logic',
      choices: [
        {
          value: 'yes',
          label: 'Yes',
...
<div class="mx-auto max-w-2xl" role="region" aria-label="Interactive decision quiz">
  ...
  <!-- Navigation buttons -->
  <nav class="mb-6 flex items-center justify-between" aria-label="Quiz navigation">
    <!-- Back button (always rendered, faded when not available) -->
    <button
      onclick={goBack}
      aria-label="Go back to previous question"
      aria-disabled={currentStep === 0 && !showResult}
      class="inline-flex cursor-pointer items-center gap-2 text-sm text-muted-foreground transition-all hover:text-foreground"
    ...
    <button
      ...
      aria-disabled={currentStep === 0 && !showResult}
      class="cursor-pointer text-sm text-muted-foreground transition-all hover:text-foreground"
      style:opacity={currentStep > 0 || showResult ? 1 : 0}
      style:pointer-events={currentStep > 0 || showResult ? 'auto' : 'none'}
    >
      Start Over
  ...
  >
    {#if !showResult}
      {#key currentQuestionIndex}
        ...
        >
          <h3 id="question-{currentQuestionIndex}" class="mb-3 text-2xl font-semibold">
            {questions[currentQuestionIndex].question}
          </h3>

          {#if questions[currentQuestionIndex].description}
            <p id="question-desc-{currentQuestionIndex}" class="mb-6 text-sm text-muted-foreground">
          ...
          >
            {#each questions[currentQuestionIndex].choices as choice}
              <button
                ...
                class="w-full cursor-pointer rounded-lg bg-muted p-4 text-left transition-all hover:bg-indigo-500/25 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none"
              >
                <div class="mb-1 font-medium">{choice.label}</div>
